...
import zq.mdlib.contextmenu.interfaces.OnItemClickListener;
import zq.mdlib.contextmenu.interfaces.OnItemLongClickListener;
...
public class MenuAdapter
{
	...
	private int mAnimationDurationMillis = ANIMATION_DURATION_MILLIS;

	public MenuAdapter(Context context, LinearLayout menuWrapper, LinearLayout textWrapper,
					   List<MenuObject> menuObjects, int actionBarHeight)
	{
	...
	private void setViews()
	{
		for (int i = 0; i < mMenuObjects.size(); i++)
		{
			...
			mMenuWrapper.addView(Utils.getImageWrapper(mContext, menuObject, mMenuItemSize, clickItem, longClickItem, false));
			// 这里可以把最后一个值设置成false
		}
	}
	...
	private AnimatorSet setOpenCloseAnimation(boolean isCloseAnimation)
	{
		...
		animatorFullSet.setDuration(mAnimationDurationMillis);
	 ...
	 */
	private void fillOpenClosingAnimations(boolean isCloseAnimation, List<Animator> textAnimations,
	...
	private void buildChosenAnimation(int childIndex)
	{
		...
		fullAnimatorSet.setDuration(mAnimationDurationMillis);
